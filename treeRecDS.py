...
# data dimensions
trials = VmTreeDF.columns.levels[0].values
directions = VmTreeDF.columns.levels[1].values
...
def plotMap(locs, theta, DSi, degMax=180, dsiMax=1, trial=-1):
    ...
    sm = plt.cm.ScalarMappable(cmap='jet',
                               norm=plt.Normalize(vmin=0, vmax=degMax))
    sm._A = []
    tFig.colorbar(sm)
    # create DSi colour bar (legend)
    sm = plt.cm.ScalarMappable(cmap='jet',
                               norm=plt.Normalize(vmin=0, vmax=dsiMax))
    sm._A = []
    dFig.colorbar(sm)
...
# create function for "line plots" of voltage along a dendrite
